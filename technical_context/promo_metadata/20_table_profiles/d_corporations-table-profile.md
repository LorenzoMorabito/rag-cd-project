# d_corporations — Table Profile

## Role
Dimension table used to slice the promotional fact.

## Structural metadata
- full name: `dev_iqvia_catalog.trusted_starschema_01.d_corporations`
- table role: `dimension`
- primary key: `CORPORATION_ID`
- row count: `53125`
- owner: `ccantamaglia@ext.menarini.com`
- data format: `DELTA`

## Semantic interpretation
Interpretation is inferred from table name and column structure because no descriptive comment was present.

## Column groups
- business keys: `CORPORATION_ID`
- measures: none
- non-key attributes: `CORPORATION`

## Inbound relationships
- used by `f_channel_dynamic_data.CORPORATION_ID` (declared_metadata_fk)

## Column inventory

|   position | column_name    | type_name   | nullable   | semantic_role       |
|-----------:|:---------------|:------------|:-----------|:--------------------|
|          0 | CORPORATION_ID | LONG        | False      | business_key        |
|          1 | CORPORATION    | STRING      | False      | attribute           |
|          2 | _trusted_ts    | TIMESTAMP   | True       | technical_timestamp |